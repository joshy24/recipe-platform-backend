...
module.exports.getInventory = async(req,res) => {
    ...
    try{
        if(type.toLowerCase() == "ingredients" || type.toLowerCase() == "ingredient"){
            ...
            console.log({...allIngredients, docs: editedList._doc})
        ...
        if(type.toLowerCase() == "materials" || type.toLowerCase() == "material"){
            ...
            console.log({...allMaterials, docs: editedList._doc})
            return res.status(200).send({response: {...allMaterials, docs: editedList._doc}})
        }
    }
    catch(err){
        console.log(err)
...
module.exports.getIngredientsToAdd = async(req,res) => {
    ...
    try{
        ...
        if(recipe.ingredients.length > 0){
            ...
            if(offset && limit){
                const ingredients_found = await IngredientService.getIngredientsNotInArray(recipe_ingredients_ids, offset, limit, req.tenantModels.ingredientModel, search_term)
